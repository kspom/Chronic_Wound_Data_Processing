# Chronic_Wound_Data_Processing
Analysis of longitudinal transcriptomic data from human chronic wounds (diabetic foot ulcers) collected on a weekly basis.
% data reading file sequence:
%   Read_Data:
%                   Output is ChronicWoundData.mat
%   BoxPlotSomeGenes:
%                   a tool for plotting desired genes
%   BatchEffectCorrection:
%                   Output is ChronicWoundDataBatchCorrected.mat
%   Clusters_analysis:
%                   deriving clusters, plotting scatterplots and
%                   cluster time-series
%   Timeseries_plot:
%                   timeseries of gene expression, non-normalized, 
%                   one cluster, each patient
%   validation_pcr_scatters:
%                   comparing RNAseq and qPCR, scatterplots
%   qpcr_validation_barplots:
%                   comparing RNAseq and qPCR, barplots   
%   PrinCompAnalysis:
%                   PCA for non-normalized gene expression data
%   markov_chain_model:
%                   Markov Chain Model for healers and non-healers
%   DEanalysis:
%                   differentially expressed genes between healers and
%                   non-healers: for all, treated and non-treated samples
%   
